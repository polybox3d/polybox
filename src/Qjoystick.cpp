    ...
    for(int i = 0; i < 10; i++) {
        ...
        switch (event.type & ~JS_EVENT_INIT)  {
            case JS_EVENT_AXIS:
                ...
                if(axes[event.number] != axisvalue) emit axisChanged(event.number+this->getInfo().buttons, axisvalue);
...
QJoystick* QJoystick::openJoystick(QString path, int msec) {
    char* p = (char*) path.toStdString().c_str(); // Get the ascii version of the path
...
JSInfo QJoystick::getJoystickInfo(QString path) {
    int fd = open(path.toStdString().c_str(), O_RDONLY); // Open the dev port
    JSInfo info;
